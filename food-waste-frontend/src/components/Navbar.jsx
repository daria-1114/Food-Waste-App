...
const Navbar = ({ setView, onLogout, userName }) => {
  return (
    <AppBar position="static" sx={{ mb: 3, backgroundColor: "#2c3e50" }}>
      <Toolbar>
        ...
        <Box sx={{ display: "flex", gap: 2 }}>
          <Button color="inherit" startIcon={<KitchenIcon />} onClick={() => setView("dashboard")}>
            My Fridge
          </Button>
          <Button color="inherit" startIcon={<ListAltIcon />} onClick={() => setView("lists")}>
            My Lists
          </Button>
          <Button color="inherit" startIcon={<GroupIcon />} onClick={() => setView("groups")}>
            Groups
          </Button>
        </Box>
